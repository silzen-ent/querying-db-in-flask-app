...
def pet_by_id(id):
    ...
    # Creating a conditional statement to check if the pet exists and if not, return a 404 status code instead of a error message that suggests something went wrong with the server.
    if pet:
...
def pets_by_species(species):
    ...
    size = len(pets) # all() returns a list so we can get length of the list to check if it's empty
